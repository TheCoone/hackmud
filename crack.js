function(context, args) //t:0
{
	...
	if (/hard/.exec(args.t.call())) { //Tests if there is no hardline
		return "No hardline"
	}
	while (1) { //Infinite loop, this will time out if you are not in hardline
		...
		var result=regex.exec(args.t.call(current_args)) //Calls the target with current argumnents at looks for regex all in one
